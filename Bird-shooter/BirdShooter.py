...
class Duck(games.Sprite):
    ...
    def __init__(self, duckType):     
        ...
        super(Duck, self).__init__(image=self.flyRight[1], x=randint(10, 1360), y=590, dx=0, dy=-1)
    ...
    def update(self):
        ...
        if not Game.paused and not Game.over:
            ...
            if self.alive:
                # Duck is alive
                if self.bottom < 0 or self.right < 0 or self.left > 1370:
                ...
                if Cursor.clicked:
                    ...
                    if not ( Cursor.yPos in range(600, 770)):
...
class Clock(games.Sprite):
    ...
    def __init__(self):
        ...
        # Timer Display
        self.timer = games.Text(value="1:00", size=50, x=880, y=650, color=color.white)
...
class Game(games.Sprite):
    ...
    scoreLabel = games.Text(value="0", size=25, left=1050, y=670, color=color.white)
    games.screen.add(scoreLabel)

    # Label for number of ducks shot
    ducksShotLabel = games.Text(value="0", size=30, x=130, y=650, color=color.white)
